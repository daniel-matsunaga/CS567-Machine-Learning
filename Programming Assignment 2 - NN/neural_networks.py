...
class linear_layer:
    ...
    def forward(self, X):
        ...
        w = np.dot(X, self.params['W'])
        forward_output = np.add(w, self.params['b'])
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        deriv = np.subtract(1, np.power(np.tanh(X), 2))
        backward_output = np.multiply(deriv, grad)
        return backward_output
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    # Update model param with new momentum
                    ...
                    # Update model param without momentum
