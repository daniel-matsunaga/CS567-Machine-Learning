...
class KNN:
    ...
    def train(self, features, labels):
        ...
        """
        # Store training data in local variables
    ...
    def predict(self, features):
        ...
        for point in features:
            label = Counter(self.get_k_neighbors(point)).most_common(1)
            predict_labels.append(label[0][0])
    ...
    def get_k_neighbors(self, point):
        ...
        for feature, label in zip(self.features, self.labels):
            distances.append([self.distance_function(feature, point), label])
            
        distances.sort(key=lambda x : x[0])
        neighbor_labels = [x[1] for x in distances[:self.k]]
